...
def pulp_solver(A_matrix: numpy.array, c: numpy.array, b: numpy.array) -> (numpy.array, float):
    ...
    x_name = [f'x_{i}' for i in range(len(c))]
    X = [pulp.LpVariable(x_name[i], lowBound = 0) for i in range(len(c)) ]
    objective = pulp.LpAffineExpression([ (X[i],c[i]) for i in range(len(c))])
    
    constraints = [pulp.LpAffineExpression([ (X[i],A_matrix[j,i]) for i in range(len(c))]) for j in range(len(A_matrix)) ]

    model += objective
    for i in range(len(constraints)):
        model += constraints[i] <= b[i]

    status = model.solve()

    opt_val = model.objective.value()
