...
def create_weights(data_dictionary, lambda_val):
    ...
    weights = np.matmul(np.matmul(np.linalg.inv(np.matmul(np.transpose(X_train),X_train) + lambda_val*np.identity(X_train.shape[1])),np.transpose(X_train)),Y_train)
...
def generate_test_error(data_dictionary, weights):
    ...
    test_error = ((np.matmul(X_test,weights) - Y_test)*(np.matmul(X_test,weights) - Y_test)).sum() / X_test.shape[0]
...
def find_optimal_lambda(error_array, lambda_array):
    ...
    min_val = error_array[0]
    min_index = 0
    for i in range(len(error_array)):
        if error_array[i] < min_val:
            min_val = error_array[i]
            min_index = i
