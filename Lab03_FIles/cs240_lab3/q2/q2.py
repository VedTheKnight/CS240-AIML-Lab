...
def lasso_loss(X, Y, w, lambd):
    ...
    loss = ((torch.matmul(X,w) - Y)*(torch.matmul(X,w) - Y)).sum() / X.shape[0] + lambd * w.apply_(lambda x : abs(x)).sum()
    ### YOUR CODE ENDS HERE ###
...
def lasso_loss_derivative(X, Y, w, lambd):
    ...
    ols_derivative = 2 * torch.matmul(X.transpose(0,1), error) / X.size(0)

    derivative = ols_derivative + lambd * torch.sign(w) 
