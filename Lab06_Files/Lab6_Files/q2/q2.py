...
class SKLearnDecisionTree:
    ...
    def train_model(self, criterion="entropy"):
        ...
        results = {}
        results["train_accuracy"] = metrics.accuracy_score(y_pred_train,self.Y_train)
        results["test_accuracy"] = metrics.accuracy_score(y_pred_test,self.Y_test)
        return results
        ### Student Code end
    ...
    def train_model_regularised(self, criterion="entropy"):
        ...
        for i in range(1,11):
            for j in range(3,31,3):
                for k in range(1,11):
                    self.tree = DecisionTreeClassifier(random_state = 42,criterion = criterion,min_samples_leaf=k,min_samples_split=j,max_depth=i)
                    self.tree.fit(self.X_train,self.Y_train)

                    y_pred_train = self.tree.predict(self.X_train)
                    y_pred_test = self.tree.predict(self.X_test)
                    train_accuracies.append(metrics.accuracy_score(y_pred_train,self.Y_train))
                    test_accuracies.append(metrics.accuracy_score(y_pred_test,self.Y_test))
