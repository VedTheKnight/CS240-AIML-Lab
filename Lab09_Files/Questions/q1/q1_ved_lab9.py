...
class ConvolutionLayer:
    ...
    def backward(self, output_error, learning_rate):
        ...
        for f in range(self.num_filters):
            for i in range(height - self.filter_size + 1):
                for j in range(width - self.filter_size + 1):
                    input_error[i:i+self.filter_size, j:j+self.filter_size] += self.conv_filter[f] * output_error[f, i, j]


        for f in range(self.num_filters):
            for i in range(height - self.filter_size + 1):
                for j in range(width - self.filter_size + 1):
                    self.conv_filter[f,:,:] -= learning_rate * output_error[f,i,j] * input[i:i+self.filter_size,j:j+self.filter_size]
...
class ReLULayer:
    ...
    def forward(self, input):
        ...
        output = None
        self.input = input
        zero = np.zeros_like(input)

        ## TODO
        output = np.maximum(zero,input)
        ## END TODO
...
class MaxPoolingLayer:
    ...
    def forward(self, input):
        ...
        ## TODO
        output = np.zeros((num_filters, height // filter_size, width // filter_size))

        for f in range(num_filters):
            for i in range(0,height,filter_size):
                for j in range(0,width,filter_size):
                    output[f,i//filter_size,j//filter_size] = np.max(input[f,i:i+filter_size,j:j+filter_size])
    ...
    def backward(self, output_error, learning_rate):
        ...
        for f in range(input.shape[0]):
            for i in range(0,input.shape[1],self.filter_size):
                for j in range(0,input.shape[2],self.filter_size): 
                    window = self.input[f, i:i+self.filter_size, j:j+self.filter_size]
                    max_idx = np.unravel_index(np.argmax(window), window.shape)
                    input_error[f, i+max_idx[0], j+max_idx[1]] = output_error[f, i//self.filter_size, j//self.filter_size]
