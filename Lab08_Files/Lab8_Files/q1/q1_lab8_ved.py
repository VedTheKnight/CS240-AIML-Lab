...
class RNN:
    ...
    def softmax(self, x):
        ...
        # x_shifted = np.exp(x_shifted - np.max(x_shifted))

        # normalize the exponentials by dividing by their sum
        x_shifted = x - np.max(x)
        p = np.exp(x_shifted) / np.sum(np.exp(x_shifted))
    ...
    def forward(self, X, a_prev):
        ...
        for t in range(len(X)):
            ...
            h = np.matmul(self.Waa,a[t-1]) + np.matmul(self.Wax, x_t) + self.ba #made x_t (vocab,1)
            a_t = np.tanh(h)
            a[t] = a_t

            # compute the output probabilities at the current time step
            y_t = self.softmax(np.matmul(self.Wya,a_t) + self.by)
            y_pred[t] = y_t 
    ...
    def backward(self,x, a, y_preds, targets):
        ...
        for t in reversed(range(len(x))):
            ...
            da = np.dot(self.Waa.T, da_next) + np.dot(self.Wya.T, dy)
            dtanh = 1 - a[t]*a[t]
            dh = dtanh * da
            ...
            self.dba += dh
            self.dWax += np.matmul(dh,x[t].T) #U x is h,1 h is v,1
            self.dWaa += np.matmul(dh , a[t-1].T) #W
    ...
    def predict(self, start):
        ...
        while(idx != newline_character and counter != max_chars):
            ...
            a = np.tanh(np.matmul(self.Wax,x) + np.matmul(self.Waa, a_prev) + self.ba)
            y_pred = self.softmax(np.matmul(self.Wya,a) + self.by)
