...
class RNN:
    ...
    def softmax(self, x):
        ...
        p = exp_x_shifted/np.sum(exp_x_shifted)
    ...
    def forward(self, X, a_prev):
        ...
        for i in range(len(X)):
            ...
            a[i] = np.tanh(np.matmul(self.Waa, a[i-1]) + np.matmul(self.Wax, x[i]) + self.ba)
            # compute the output probabilities at the current time step
            y_pred[i] = self.softmax(np.matmul(self.Wya, a[i]) + self.by)
