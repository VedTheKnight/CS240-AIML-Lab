...
class MultiClassLogisticRegression:
    ...
    def predict(self, X):
        ...
        pre_vals = np.matmul(X , np.transpose(self.weights)) 
        # print(f"predicted vals {pre_vals.shape}")
    #   ## YOUR CODE ENDS HERE ###
    ...
    def softmax(self, z):
        ...
        for i in range(post_softmax.shape[0]):
            post_softmax[i,:] = post_softmax[i,:] / post_softmax[i,:].sum()

        # print(post_softmax)
        # print(f"Post soft max {post_softmax.shape}")
        ### YOUR CODE ENDS HERE ###
        return post_softmax
    ...
    def cross_entropy(self, y, probs):
        ...
        post_cross_entropy = (-1 * (np.log(probs) * y) / probs.shape[0]).sum()
        ### YOUR CODE ENDS HERE ###
