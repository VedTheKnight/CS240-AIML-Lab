...
def cost_logreg(X, y, w, w_0):
    ...
    cost = log_loss(y,y_dash).mean()

    #cost = (np.log(1 + np.exp(-1*(np.matmul(X,w) + w_0))) + (1-y)*(np.matmul(X,w) + w_0)).mean()
    ### YOUR CODE ENDS HERE ###
...
def grad_logreg(X, y, w, w_0):
    ...
    ### YOUR CODE BEGINS HERE ###
    y_dash = logistic(np.matmul(X,w) + w_0)
    grad_w = np.transpose(np.matmul(np.transpose(y_dash - y),X)) / m
    grad_w_0 = np.transpose(y_dash - y).mean()
    ### YOUR CODE ENDS HERE ###
